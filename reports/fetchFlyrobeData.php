...
if ($row["productType"] == 2) {
    ...
    if (strtotime($billDate) < strtotime("2025-09-22")) {
        ...
        $gst_rate = ($row["totalProductAmount"] > 2500) ? 18 : 5;
    }
} else {
    // Jewellery
    $gst_rate = 3;
...
$thisProductTotalTaxable = $row["totalProductAmount"] / (1 + ($gst_rate / 100));
$thisProductTotalGst = round($thisProductTotalTaxable * ($gst_rate / 100), 2);


                
                $ssfs = $row["netAmount"] - $row["commision_amount"];
                $new_bill_number = ($row['new_bill_number'] ?  $row['new_bill_number'] : $row["purchase_id"]);
...
<tr>
    ...
    <td style="white-space:nowrap;"><a href="./rent_report_detail.php?id=' . $row["purchase_id"] . '" target="_blank">' . $new_bill_number . '</a></td>
  ...
  <td>' . 
        ($row["productType"] == 2 
            ? (strtotime($billDate) < strtotime("2025-09-22") 
                ? "12%" 
                : ($row["totalProductAmount"] > 2500 ? "18%" : "5%")) 
            : "3%") 
    . '</td>
