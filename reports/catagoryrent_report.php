...
// -------------------------------------------------------------
// AJAX ENDPOINT: Fetch Rental History for a Specific Item SKU
// -------------------------------------------------------------
if (isset($_GET['action']) && $_GET['action'] === 'item_history') {
    header('Content-Type: application/json');
    $sku = safe_str($_GET['sku'] ?? '');
    if (!$con || $sku === '') {
        echo json_encode(['success' => false, 'message' => 'Invalid request or SKU.']);
        exit;
    }
    ...
    if ($q_meta && $row_meta = mysqli_fetch_assoc($q_meta)) {
        $item_meta = $row_meta;
    }
    ...
    $total_rent_item = 0;
    $total_qty_item = 0;
    $total_deposit_item = 0;

    if ($res_hist) {
        while ($row = mysqli_fetch_assoc($res_hist)) {
            $total_rent_item += floatval($row['rent'] ?? 0);
            $total_qty_item += intval($row['qty'] ?? 0);
            $total_deposit_item += floatval($row['deposit'] ?? 0);
            $history[] = [
                'bill_id'         => $row['bill_id'],
                'bill_no'         => !empty($row['new_bill_number']) ? $row['new_bill_number'] : ('BILL-' . $row['bill_id']),
                'bill_date'       => $row['bill_date'] ? date('d M Y', strtotime($row['bill_date'])) : '—',
    ...
    echo json_encode([
        ...
        'total_deposit'  => $total_deposit_item,
        'history'        => $history
    ]);
    exit;
}
...
// Overall Store KPI baseline
$kpi_store_revenue  = 0;
$kpi_store_bookings = 0;
$kpi_store_units    = 0;
$kpi_store_cats     = count($categories_list);

if ($con) {
    $q_store_kpi = mysqli_query($con, "
        SELECT 
            COUNT(DISTINCT od.bill_id) as total_bookings,
            COALESCE(SUM(od.qty), 0) as total_units,
            COALESCE(SUM(od.rent), 0) as total_revenue
        FROM order_detail od
        JOIN phppos_rent r ON od.bill_id = r.bill_id
    ");
    if ($q_store_kpi && $rk = mysqli_fetch_assoc($q_store_kpi)) {
        $kpi_store_revenue  = floatval($rk['total_revenue'] ?? 0);
        $kpi_store_bookings = intval($rk['total_bookings'] ?? 0);
        $kpi_store_units    = intval($rk['total_units'] ?? 0);
    }
}
...
if ($con) {
    ...
    if (!empty($from_date) && !empty($to_date)) {
        $date_conditions[] = "r.bill_date BETWEEN '$from_date' AND '$to_date'";
    } elseif (!empty($from_date)) {
        $date_conditions[] = "r.bill_date >= '$from_date'";
    } elseif (!empty($to_date)) {
        $date_conditions[] = "r.bill_date <= '$to_date'";
    }
    ...
    if ($search_query !== '') {
        ...
        $search_sql = " AND (od.item_id LIKE '%$q_esc%' OR i.description LIKE '%$q_esc%')";
    }

    if ($is_specific_category) {
        // ITEM-LEVEL BREAKDOWN FOR SELECTED CATEGORY
        $cat_esc = mysqli_real_escape_string($con, $selected_cat);

        // 1. Fast Full-Category KPI & Total Count Query
        $sql_cat_summary = "
            SELECT 
                COUNT(DISTINCT od.item_id) as total_items,
                COUNT(DISTINCT od.bill_id) as total_bookings,
            ...
            WHERE i.category = '$cat_esc' $date_sql $search_sql
        ";
        ...
        if ($res_c_sum && $rcs = mysqli_fetch_assoc($res_c_sum)) {
            ...
            $summary_stats['total_rent']     = floatval($rcs['total_rent'] ?? 0);
            $summary_stats['total_deposit']  = floatval($rcs['total_deposit'] ?? 0);
            $summary_stats['total_qty']      = intval($rcs['total_qty'] ?? 0);
            $summary_stats['total_bookings'] = intval($rcs['total_bookings'] ?? 0);
        ...
        // CATEGORY LEADERBOARD / ALL CATEGORIES OVERVIEW
        $sql_cat_lead = "
            SELECT 
                i.category,
                COUNT(DISTINCT od.bill_id) as booking_count,
                ...
                COALESCE(SUM(od.rent), 0) as total_rent,
                COALESCE(SUM(od.deposit), 0) as total_deposit,
                COUNT(DISTINCT od.item_id) as unique_items
            FROM order_detail od
            JOIN phppos_rent r ON od.bill_id = r.bill_id
            JOIN phppos_items i ON od.item_id = i.name
            WHERE i.category != '' AND i.category IS NOT NULL $date_sql
            GROUP BY i.category
        ...
        if ($res_cats) {
            while ($row = mysqli_fetch_assoc($res_cats)) {
                $report_rows[] = $row;
                $summary_stats['total_rent']     += floatval($row['total_rent'] ?? 0);
                $summary_stats['total_deposit']  += floatval($row['total_deposit'] ?? 0);
                $summary_stats['total_qty']      += intval($row['total_qty'] ?? 0);
                $summary_stats['total_bookings'] += intval($row['booking_count'] ?? 0);
...
$avg_rent_per_unit = ($summary_stats['total_qty'] > 0) ? ($summary_stats['total_rent'] / $summary_stats['total_qty']) : 0;
$avg_rent_per_booking = ($summary_stats['total_bookings'] > 0) ? ($summary_stats['total_rent'] / $summary_stats['total_bookings']) : 0;

// Pagination URL Builder Helper
function get_rent_page_url($p) {
    $params = $_GET;
    $params['page'] = $p;
    return 'catagoryrent_report.php?' . http_build_query($params);
}
...
<div class="container-fluid page-body-wrapper">
    ...
    <div class="main-panel">
        <div class="content-wrapper" style="padding: 1.5rem 1.75rem !important; background: #f8fafc; min-height: calc(100vh - 58px);">
            ...
            <div class="catrent-container">

                <!-- Page Header -->
                <div class="pm-page-header">
                    <div>
                        <h1 class="pm-page-title">
                            <i class="fa-solid fa-layer-group" style="margin-right: 10px; font-size: 18px; color: var(--pm-slate-700);"></i>
                            Consolidated Category Rent Report
                        </h1>
                        <p class="pm-page-subtitle">
                            Analyze rental performance, bookings volume, and inventory utilization across jewelry and apparel categories.
                        </p>
                    </div>
                ...
                <div class="pm-kpi-grid">
                    <div class="pm-kpi-card">
                        <div class="pm-kpi-header">
                            <span class="pm-kpi-label"><?= $is_specific_category ? 'Category Rental Revenue' : 'Scope Rental Revenue' ?></span>
                            <div class="pm-kpi-icon"><i class="fa fa-inr"></i></div>
                        </div>
                        <div class="pm-kpi-value">₹ <?= number_format($summary_stats['total_rent']) ?></div>
                        <div class="pm-kpi-sub">
                            <?= $is_specific_category ? ('Category: ' . htmlspecialchars($selected_cat)) : ('Across ' . $summary_stats['item_count'] . ' active categories') ?>
                        </div>
                    </div>

                    <div class="pm-kpi-card">
                        <div class="pm-kpi-header">
                            <span class="pm-kpi-label">Units Rented (Dispatches)</span>
                            <div class="pm-kpi-icon"><i class="fa fa-cubes"></i></div>
                        </div>
                        <div class="pm-kpi-value"><?= number_format($summary_stats['total_qty']) ?></div>
                        <div class="pm-kpi-sub">Total rental pieces dispatched</div>
                    </div>
                    ...
                    <div class="pm-kpi-card">
                        ...
                        <div class="pm-kpi-value"><?= number_format($summary_stats['total_bookings']) ?></div>
                        <div class="pm-kpi-sub">Total rental bills generated</div>
                    </div>

                    <div class="pm-kpi-card">
                        <div class="pm-kpi-header">
                            <span class="pm-kpi-label"><?= $is_specific_category ? 'Avg Rent / Piece' : 'Deposit Handled' ?></span>
                            <div class="pm-kpi-icon"><i class="fa <?= $is_specific_category ? 'fa-tag' : 'fa-shield' ?>"></i></div>
                        </div>
                        <div class="pm-kpi-value">
                            <?= $is_specific_category ? ('₹ ' . number_format($avg_rent_per_unit)) : ('₹ ' . number_format($summary_stats['total_deposit'])) ?>
                        </div>
                        <div class="pm-kpi-sub">
                            <?= $is_specific_category ? ('Avg per booking: ₹ ' . number_format($avg_rent_per_booking)) : 'Security deposit recorded' ?>
                        </div>
                    </div>
                </div>
                ...
                <div class="pm-card">
                    <div class="pm-card-body">
                        <form method="GET" action="catagoryrent_report.php" id="filterForm">
                            <div class="pm-filter-grid">
                                <div class="pm-form-group">
                                    ...
                                    <select name="cate" id="catSelect" class="pm-select">
                                        <option value="all" <?= (!$is_specific_category || $selected_cat === 'all') ? 'selected' : '' ?>>
                                            All Categories (Store Leaderboard)
                                        </option>
                                ...
                                <div class="pm-form-group">
                                    ...
                                    <input type="text" name="q" id="filterQ" class="pm-input" 
                                           placeholder="e.g. 101, L102, necklace..." 
                                ...
                                <div style="display: flex; gap: 8px;">
                                    ...
                                    <a href="catagoryrent_report.php" class="pm-btn pm-btn-secondary" title="Reset Filters">
                ...
                <div class="pm-banner-card">
                    <div class="pm-banner-title">
                        ...
                        <div style="font-size: 13px; color: var(--pm-slate-600);">
                            Found <strong><?= number_format($total_items_in_scope) ?></strong> matching rented items in this category.
                    ...
                    <div>
                        <a href="catagoryrent_report.php" class="pm-btn pm-btn-secondary pm-btn-sm">
                ...
                <div class="pm-card" style="margin-bottom: 32px;">
                            ...
                            <?php if ($is_specific_category): ?>
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center" style="width: 110px;">Bookings</th>
                                        <th class="text-center" style="width: 90px;">Rented Qty</th>
                                        <th class="text-right" style="width: 130px;">Total Rent (₹)</th>
                                        ...
                                        ?>
                                            <tr data-search="<?= htmlspecialchars($search_text) ?>">
                                                ...
                                                <td class="text-right font-mono">
                                                    <?= floatval($row['unit_price']) > 0 ? ('₹ ' . number_format($row['unit_price'])) : '—' ?>
                                                </td>
                                                <td class="text-center font-mono">
                                                    <strong><?= intval($row['booking_count'] ?? 0) ?></strong>
                                                </td>
                                                <td class="text-center font-mono">
                                                    <?= intval($row['total_qty'] ?? 0) ?>
                                                </td>
                                                <td class="text-right font-mono" style="font-weight: 700; color: var(--pm-slate-900);">
                                                    ₹ <?= number_format($row['total_rent'] ?? 0) ?>
                                                </td>
                                                <td class="text-right font-mono">
                                                    <?= floatval($row['total_deposit']) > 0 ? ('₹ ' . number_format($row['total_deposit'])) : '—' ?>
                                                </td>
                                                <td class="text-center">
                                                    <button type="button" class="pm-btn pm-btn-secondary pm-btn-sm" 
                                                            onclick="openItemRentalHistory('<?= htmlspecialchars(addslashes($row['item_id'])) ?>');"
                                                            title="View individual customer bookings">
                                ...
                                <tfoot class="pm-table-footer">
                                    <tr>
                                        ...
                                        <td class="text-center font-mono"><?= number_format($summary_stats['total_bookings']) ?></td>
                                        <td class="text-center font-mono"><?= number_format($summary_stats['total_qty']) ?></td>
                                        <td class="text-right font-mono">₹ <?= number_format($summary_stats['total_rent']) ?></td>
                                        <td class="text-right font-mono">₹ <?= number_format($summary_stats['total_deposit']) ?></td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                                <?php endif; ?>
                            ...
                            <?php else: ?>
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center" style="width: 120px;">Bookings</th>
                                        <th class="text-center" style="width: 120px;">Units Rented</th>
                                        <th class="text-right" style="width: 160px;">Total Rent Revenue</th>
                                    ...
                                    <?php if (empty($report_rows)): ?>
                                        <tr>
                                            <td colspan="8" class="text-center" style="padding: 48px; color: var(--pm-slate-400);">
                                                <i class="fa fa-inbox" style="font-size: 32px; margin-bottom: 12px; display: block;"></i>
                                                No category rental records found for the selected date range.
                                            </td>
                                        </tr>
                                    <?php else: ?>
                                        <?php 
                                        $rank = 1; 
                                        $total_rent_universe = ($summary_stats['total_rent'] > 0) ? $summary_stats['total_rent'] : 1;
                                        foreach ($report_rows as $cat_row): 
                                            $cat_name = $cat_row['category'];
                                            $share_pct = round((floatval($cat_row['total_rent']) / $total_rent_universe) * 100, 1);
                                        ...
                                        ?>
                                            <tr data-search="<?= htmlspecialchars($cat_search) ?>">
                                                ...
                                                <td>
                                                    <a href="catagoryrent_report.php?cate=<?= urlencode($cat_name) ?>&frmdate=<?= urlencode($from_date) ?>&todate=<?= urlencode($to_date) ?>" 
                                                ...
                                                <td class="text-center font-mono">
                                                    <?= number_format($cat_row['booking_count'] ?? 0) ?>
                                                </td>
                                                <td class="text-center font-mono">
                                                    <?= number_format($cat_row['total_qty'] ?? 0) ?>
                                                </td>
                                                <td class="text-right font-mono" style="font-weight: 700; color: var(--pm-slate-900);">
                                                    ₹ <?= number_format($cat_row['total_rent'] ?? 0) ?>
                                                </td>
                                                <td>
                    ...
                    <div class="pm-pagination-bar">
                        ...
                        <div class="pm-page-links">
                            <a href="<?= get_rent_page_url(1) ?>" class="pm-page-link <?= ($current_page <= 1) ? 'disabled' : '' ?>" title="First Page">
                                <i class="fa fa-angle-double-left"></i>
                            </a>
                            <a href="<?= get_rent_page_url($current_page - 1) ?>" class="pm-page-link <?= ($current_page <= 1) ? 'disabled' : '' ?>" title="Previous Page">
                            ...
                            ?>
                                <a href="<?= get_rent_page_url($p) ?>" class="pm-page-link <?= ($p === $current_page) ? 'active' : '' ?>">
                                    <?= $p ?>
                                </a>
                            <?php endfor; ?>
                            ...
                            <a href="<?= get_rent_page_url($current_page + 1) ?>" class="pm-page-link <?= ($current_page >= $total_pages) ? 'disabled' : '' ?>" title="Next Page">
                                <i class="fa fa-angle-right"></i>
                            </a>
                            <a href="<?= get_rent_page_url($total_pages) ?>" class="pm-page-link <?= ($current_page >= $total_pages) ? 'disabled' : '' ?>" title="Last Page">
...
// Open Item Rental History Modal
function openItemRentalHistory(sku) {
    ...
    fetch('catagoryrent_report.php?action=item_history&sku=' + encodeURIComponent(sku))
...
function exportReportToCSV() {
    ...
    downloadLink.download = "Category_Rent_Report_" + (new Date().toISOString().slice(0, 10)) + ".csv";
