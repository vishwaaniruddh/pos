...
// -------------------------------------------------------------
// AJAX ENDPOINT: Fetch Detailed Sales Ledger for an Item SKU
// -------------------------------------------------------------
if (isset($_GET['action']) && $_GET['action'] === 'item_sales_ledger') {
    ...
    $sql_hist = "
        SELECT 
            ...
            ad.qty,
            ad.return_qty,
            (ad.qty - COALESCE(ad.return_qty, 0)) as sold_qty,
            ad.amount,
            CASE WHEN ad.qty > 0 THEN (ad.qty - COALESCE(ad.return_qty, 0)) * ad.amount / ad.qty ELSE 0 END as net_amount,
            COALESCE(ad.discount, a.discount_per, '—') as discount,
            cust.person_id,
            cust.first_name,
            cust.last_name,
            cust.phone_number
        FROM approval_detail ad
        JOIN approval a ON ad.bill_id = a.bill_id
        LEFT JOIN phppos_people cust ON a.cust_id = cust.person_id
        WHERE ad.item_id = '$sku_esc'
    ...
    $total_sales_item    = 0;
    $total_sold_item     = 0;
    $total_approval_item = 0;

    if ($res_hist) {
        while ($row = mysqli_fetch_assoc($res_hist)) {
            $is_sale = (strtolower($row['status'] ?? '') === 's');
            $net_a   = floatval($row['net_amount'] ?? 0);
            $sold_q  = max(0, intval($row['sold_qty'] ?? 0));

            if ($is_sale) {
                $total_sales_item += $net_a;
                $total_sold_item  += $sold_q;
            } else {
                $total_approval_item += $net_a;
            }

            $cust_name = trim(($row['first_name'] ?? '') . ' ' . ($row['last_name'] ?? ''));
            if ($cust_name === '') $cust_name = !empty($row['person_id']) ? ('Customer #' . $row['person_id']) : 'Guest Customer';
    ...
    echo json_encode([
        ...
        'total_approval' => $total_approval_item,
        'history'        => $history
    ]);
    exit;
}
...
// -------------------------------------------------------------
$search_query = safe_str($_GET['q'] ?? ($_GET['item_id'] ?? ($_GET['sku'] ?? '')));
$selected_cat = safe_str($_GET['category'] ?? ($_GET['cate'] ?? ''));
$trans_type   = safe_str($_GET['status'] ?? 's'); // Defaults to Sales ('s')
...
$where_clauses = ["ad.item_id != ''", "ad.item_id IS NOT NULL", "ad.qty > 0"];
...
if (!empty($selected_cat) && $selected_cat !== 'all') {
    ...
    $q_cat_skus = mysqli_query($con, "SELECT name FROM phppos_items WHERE category = '$cat_esc'");
    $c_skus = [];
    if ($q_cat_skus) {
        while ($rc = mysqli_fetch_row($q_cat_skus)) {
            $c_skus[] = "'" . mysqli_real_escape_string($con, $rc[0]) . "'";
        }
    }
    if (!empty($c_skus)) {
        $where_clauses[] = "ad.item_id IN (" . implode(',', $c_skus) . ")";
    } else {
        $where_clauses[] = "1=0";
    }
}
...
if ($search_query !== '') {
    ...
    $q_desc_items = mysqli_query($con, "SELECT name FROM phppos_items WHERE name LIKE '%$q_esc%' OR description LIKE '%$q_esc%'");
    $desc_skus = [];
    if ($q_desc_items) {
        while ($rd = mysqli_fetch_row($q_desc_items)) {
            $desc_skus[] = "'" . mysqli_real_escape_string($con, $rd[0]) . "'";
        }
    }
    if (!empty($desc_skus)) {
        $where_clauses[] = "(ad.item_id LIKE '%$q_esc%' OR ad.item_id IN (" . implode(',', $desc_skus) . "))";
    } else {
        $where_clauses[] = "ad.item_id LIKE '%$q_esc%'";
    }
}
...
// Phase 1: Fast Full-Scope KPI Summary & Distinct Items Count
...
if ($con) {
    ...
    if ($res_sum && $rsum = mysqli_fetch_assoc($res_sum)) {
        $total_items_in_scope          = intval($rsum['total_items'] ?? 0);
        $summary_stats['total_sales']   = floatval($rsum['total_sales_amt'] ?? 0);
        $summary_stats['total_sold_qty'] = intval($rsum['total_sold_qty'] ?? 0);
        $summary_stats['total_bills']   = intval($rsum['total_bills'] ?? 0);
    }
}

// Phase 2: Fetch Paginated Slice of Items
$report_rows = [];
$item_meta_map = [];

if ($con && $total_items_in_scope > 0) {
    ...
    $sql_items = "
        ...
        WHERE $where_sql
        GROUP BY ad.item_id
        ORDER BY total_sales_amt DESC, total_sold_qty DESC
        $limit_clause
    ";
    ...
    if ($res_items) {
        while ($row = mysqli_fetch_assoc($res_items)) {
            $report_rows[] = $row;
            $sku_names[] = "'" . mysqli_real_escape_string($con, $row['item_id']) . "'";
        }
    }

    // Pre-fetch metadata for the paginated slice
    if (!empty($sku_names)) {
        $q_meta = mysqli_query($con, "SELECT name, category, cost_price, unit_price, description FROM phppos_items WHERE name IN (" . implode(',', $sku_names) . ")");
        if ($q_meta) {
            while ($rm = mysqli_fetch_assoc($q_meta)) {
                $item_meta_map[$rm['name']] = $rm;
            }
        }
    }
}
...
$avg_sale_per_item = ($summary_stats['total_sold_qty'] > 0) ? ($summary_stats['total_sales'] / $summary_stats['total_sold_qty']) : 0;
...
if (!function_exists('get_item_sales_page_url')) {
    function get_item_sales_page_url($p) {
        $params = $_GET;
        $params['page'] = $p;
        return 'item_sales_report.php?' . http_build_query($params);
    }
}
...
<div class="container-fluid page-body-wrapper">
    ...
    <div class="main-panel">
        <div class="content-wrapper" style="padding: 1.5rem 1.75rem !important; background: #f8fafc; min-height: calc(100vh - 58px);">
            ...
            <div class="itemsales-container">

                <!-- Page Header -->
                <div class="pm-page-header">
                    <div>
                        <h1 class="pm-page-title">
                            <i class="fa-solid fa-barcode" style="margin-right: 10px; font-size: 18px; color: var(--pm-slate-700);"></i>
                            Item Sales Report
                        </h1>
                        <p class="pm-page-subtitle">
                            Analyze individual SKU retail performance, customer transactions ledger, and inventory sales turnover.
                        </p>
                    </div>
                ...
                <div class="pm-kpi-grid">
                    <div class="pm-kpi-card">
                        ...
                        <div class="pm-kpi-sub">
                            Across <?= number_format($total_items_in_scope) ?> matching SKUs in scope
                        </div>
                    </div>

                    <div class="pm-kpi-card">
                        <div class="pm-kpi-header">
                            <span class="pm-kpi-label">Unique SKUs in Scope</span>
                            <div class="pm-kpi-icon"><i class="fa fa-tags"></i></div>
                        </div>
                        <div class="pm-kpi-value"><?= number_format($total_items_in_scope) ?></div>
                        <div class="pm-kpi-sub">Total distinct item designs sold</div>
                    </div>
                    ...
                    <div class="pm-kpi-card">
                        ...
                        <div class="pm-kpi-sub">Net units sold after return adjustments</div>
                    </div>

                    <div class="pm-kpi-card">
                        <div class="pm-kpi-header">
                            <span class="pm-kpi-label">Avg Realized / Piece</span>
                            <div class="pm-kpi-icon"><i class="fa fa-calculator"></i></div>
                        </div>
                        <div class="pm-kpi-value">₹ <?= number_format($avg_sale_per_item) ?></div>
                        <div class="pm-kpi-sub">
                ...
                <div class="pm-card">
                    <div class="pm-card-body">
                        <form method="GET" action="item_sales_report.php" id="filterForm">
                            <div class="pm-filter-grid">
                                <div class="pm-form-group">
                                    <label class="pm-label" for="filterQ">Search SKU / Barcode</label>
                                    <input type="text" name="q" id="filterQ" class="pm-input" 
                                           placeholder="e.g. T699EON, ND185, Bhoomi..." 
                                           value="<?= htmlspecialchars($search_query) ?>">
                                </div>

                                <div class="pm-form-group">
                                    <label class="pm-label" for="catSelect">Category</label>
                                    <select name="category" id="catSelect" class="pm-select">
                                        <option value="all" <?= ($selected_cat === 'all' || empty($selected_cat)) ? 'selected' : '' ?>>
                                            All Categories
                                        </option>
                                ...
                                <div class="pm-form-group" style="max-width: 140px;">
                                    <label class="pm-label" for="typeSelect">Type</label>
                                    <select name="status" id="typeSelect" class="pm-select">
                                        <option value="s" <?= ($trans_type === 's') ? 'selected' : '' ?>>Sold (Outright)</option>
                                        <option value="a" <?= ($trans_type === 'a') ? 'selected' : '' ?>>Approval Only</option>
                                ...
                                <div style="display: flex; gap: 8px;">
                                    ...
                                    <a href="item_sales_report.php" class="pm-btn pm-btn-secondary" title="Reset Filters">
                ...
                <div class="pm-card" style="margin-bottom: 32px;">
                    ...
                    <div class="pm-table-toolbar">
                        ...
                        <div style="font-size: 12px; color: var(--pm-slate-500);">
                            <?php if (!$is_unlimited && $total_items_in_scope > $per_page): ?>
                        ...
                        <table class="pm-table" id="reportTable">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-right" style="width: 140px;">Sales Amount (₹)</th>
                                    <th style="width: 130px;">Latest Sale</th>
                            ...
                            <tbody>
                                ...
                                <?php else: ?>
                                    ...
                                    foreach ($report_rows as $row): 
                                        $sku = $row['item_id'];
                                        $meta = $item_meta_map[$sku] ?? [];
                                        $cat = !empty($meta['category']) ? $meta['category'] : 'Uncategorized';
                                        $desc = !empty($meta['description']) ? $meta['description'] : $sku;
                                        $s_amt = floatval($row['total_sales_amt'] ?? 0);
                                        $s_qty = intval($row['total_sold_qty'] ?? 0);
                                        $u_prc = floatval($meta['unit_price'] ?? 0);
                                        $search_text = strtolower($sku . ' ' . $cat . ' ' . $desc);
                                    ?>
                                        <tr data-search="<?= htmlspecialchars($search_text) ?>">
                                            ...
                                            <td>
                                                ...
                                                <div style="font-size: 11px; color: var(--pm-slate-500); margin-top: 2px;">
                                                    <span class="pm-badge-neutral"><i class="fa fa-folder-open" style="font-size: 10px;"></i> <?= htmlspecialchars($cat) ?></span>
                                                </div>
                                            </td>
                                            <td class="text-right font-mono">
                                                <?= $u_prc > 0 ? ('₹ ' . number_format($u_prc)) : '—' ?>
                                            </td>
                                            ...
                                            <td class="text-center">
                                                <button type="button" class="pm-btn pm-btn-secondary pm-btn-sm" 
                                                        onclick="openItemSalesLedger('<?= htmlspecialchars(addslashes($sku)) ?>');"
                                                        title="View customer purchase transactions">
                            ...
                            <tfoot class="pm-table-footer">
                                <tr>
                                    <td colspan="4" class="text-right">Scope Total (<?= number_format($total_items_in_scope) ?> Items):</td>
                    ...
                    <div class="pm-pagination-bar">
                        ...
                        <div class="pm-page-links">
                            <a href="<?= get_item_sales_page_url(1) ?>" class="pm-page-link <?= ($current_page <= 1) ? 'disabled' : '' ?>" title="First Page">
                                <i class="fa fa-angle-double-left"></i>
                            </a>
                            <a href="<?= get_item_sales_page_url($current_page - 1) ?>" class="pm-page-link <?= ($current_page <= 1) ? 'disabled' : '' ?>" title="Previous Page">
                            ...
                            ?>
                                <a href="<?= get_item_sales_page_url($p) ?>" class="pm-page-link <?= ($p === $current_page) ? 'active' : '' ?>">
                                    <?= $p ?>
                                </a>
                            <?php endfor; ?>
                            ...
                            <a href="<?= get_item_sales_page_url($current_page + 1) ?>" class="pm-page-link <?= ($current_page >= $total_pages) ? 'disabled' : '' ?>" title="Next Page">
                                <i class="fa fa-angle-right"></i>
                            </a>
                            <a href="<?= get_item_sales_page_url($total_pages) ?>" class="pm-page-link <?= ($current_page >= $total_pages) ? 'disabled' : '' ?>" title="Last Page">
...
// Open Item Sales Ledger Modal
function openItemSalesLedger(sku) {
    var modal = document.getElementById('itemSalesModal');
    var modalBody = document.getElementById('modalItemBody');
    var title = document.getElementById('modalSkuTitle');

    title.innerHTML = 'Sales Ledger for SKU: <span class="pm-sku-pill">' + sku + '</span>';
    modalBody.innerHTML = '<div style="text-align:center; padding:40px; color:var(--pm-slate-400);"><i class="fa fa-circle-notch fa-spin fa-2x"></i><p style="margin-top:10px; font-size:13px;">Loading item sales records...</p></div>';
    modal.style.display = 'flex';

    fetch('item_sales_report.php?action=item_sales_ledger&sku=' + encodeURIComponent(sku))
        ...
        .then(function(data) {
            ...
            var m = data.meta || {};
            var itemDesc = m.description || sku;
            var category = m.category || 'General';
            var unitPrice = Number(m.unit_price || 0);
            ...
            html += '  <div>';
            html += '    <div style="font-weight:700; font-size:15px; color:var(--pm-slate-900);">' + itemDesc + '</div>';
            html += '    <div style="font-size:12px; color:var(--pm-slate-500); margin-top:2px; display:flex; align-items:center; gap:8px;">';
            html += '      <span class="pm-sku-pill">' + sku + '</span>';
            html += '      <span><i class="fa fa-folder-open"></i> ' + category + '</span>';
            if (unitPrice > 0) html += ' &bull; <span>Retail: ₹ ' + unitPrice.toLocaleString('en-IN') + '</span>';
            html += '    </div>';
            html += '  </div>';
            html += '  <div style="display:flex; gap:16px;">';
            html += '    <div style="text-align:right;">';
            html += '      <div style="font-size:11px; font-weight:600; color:var(--pm-slate-400); text-transform:uppercase;">Realized Sales</div>';
            ...
            html += '      <div style="font-size:11px; font-weight:600; color:var(--pm-slate-400); text-transform:uppercase;">Sold Pieces</div>';
            ...
            if (!data.history || data.history.length === 0) {
                html += '<div style="text-align:center; padding:30px; color:var(--pm-slate-400);">No sales invoices recorded for this SKU.</div>';
            } else {
                ...
                html += '<th class="text-center">Qty Sold</th>';
                ...
                data.history.forEach(function(item, idx) {
                    ...
                    html += '<td>';
                    html += '  <div style="font-weight:600; color:var(--pm-slate-900);">' + item.cust_name + '</div>';
                    if (item.cust_phone && item.cust_phone !== '—') {
                        html += '  <div style="font-size:11px; color:var(--pm-slate-500);"><i class="fa fa-phone"></i> ' + item.cust_phone + '</div>';
                    }
                    html += '</td>';
                    html += '<td class="text-center"><span class="pm-status-pill">' + item.status_label + '</span></td>';
                    html += '<td class="text-center font-mono">' + item.sold_qty + '</td>';
                    ...
                    html += '<td class="text-center"><a href="/pos/reports/sales_report_detail.php?id=' + item.bill_id + '" target="_blank" class="pm-btn pm-btn-secondary pm-btn-sm" style="font-size:11px; padding:2px 8px; height:26px;"><i class="fa fa-external-link"></i> View Bill</a></td>';
                    html += '</tr>';
                });
...
function exportReportToCSV() {
    ...
    downloadLink.download = "Item_Sales_Report_" + (new Date().toISOString().slice(0, 10)) + ".csv";
