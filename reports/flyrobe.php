...
// Find latest available month and year in database if not submitted
$latest_month = date('M');
$latest_year  = date('Y');
if ($con) {
...
if ($search_query !== '') {
    ...
    $where_clauses[] = "(a.sku LIKE '%$q_esc%' OR r.new_bill_number LIKE '%$q_esc%' OR a.purchase_id LIKE '%$q_esc%' OR p.first_name LIKE '%$q_esc%' OR p.last_name LIKE '%$q_esc%' OR p.phone_number LIKE '%$q_esc%')";
}

$where_sql = implode(" AND ", $where_clauses);

// Helper function for GST and SSFS calculation
if (!function_exists('calculate_flyrobe_item_metrics')) {
    function calculate_flyrobe_item_metrics($row) {
        $billDate = $row['bill_date'];
        $rentAmount = floatval($row['totalProductAmount'] ?? 0);
        $productType = intval($row['productType'] ?? 1);
        ...
        $taxable = $rentAmount / (1 + ($gst_rate / 100));
...
if ($con) {
    $sql_all = "
        SELECT a.id, a.productType, a.totalProductAmount, a.netAmount, a.commision_amount, 
               a.beautician_discountAmount, r.bill_date
        FROM flyrob_commission a
        JOIN phppos_rent r ON a.purchase_id = r.bill_id
    ...
    if ($res_sum) {
        ...
        while ($r = mysqli_fetch_assoc($res_sum)) {
            $m = calculate_flyrobe_item_metrics($r);
            $summary['total_rent']       += $m['rent_amt'];
            $summary['total_gst']        += $m['gst_amt'];
            $summary['total_beautician'] += floatval($r['beautician_discountAmount'] ?? 0);
...
if (isset($_REQUEST['export']) && $_REQUEST['export'] == '1') {
    if ($con) {
        $export_sql = "
            SELECT a.*, r.bill_date, r.cust_id, r.pick_date, r.delivery_date, r.new_bill_number,
        ...
        if ($export_res) {
            $filename = "Flyrobe_Commission_" . ($selected_month ? $selected_month . "_" : "") . ($selected_year ? $selected_year . "_" : "") . date('Ymd_His') . ".csv";
            ...
            fputcsv($output, [
                ...
                'Pick-up Date', 'Return Date', 'Rent Amount (₹)', 'GST Rate', 'GST Amount (₹)',
                'Beautician Discount Given', 'Beautician Rate', 'Beautician Discount (₹)',
            ...
            while ($erow = mysqli_fetch_assoc($export_res)) {
                $m = calculate_flyrobe_item_metrics($erow);
                ...
                fputcsv($output, [
                    ...
                    $m['gst_rate'] . '%',
                    $m['gst_amt'],
                    $erow['isBeauticianDiscountGiven'] == 1 ? 'Yes' : 'No',
                    $erow['beautician_discountRate'] ? ($erow['beautician_discountRate'] . $erow['beautician_discountType']) : '—',
                    floatval($erow['beautician_discountAmount'] ?? 0),
                    $m['net_amt'],
...
// Query distinct years available in records
$years_list = [];
if ($con) {
...
if (!function_exists('get_flyrobe_page_url')) {
    function get_flyrobe_page_url($p) {
        $params = $_GET;
        $params['page'] = $p;
        return 'flyrobe.php?' . http_build_query($params);
    }
}
?>
            ...
            <div class="flyrobe-container">
                ...
                <div class="pm-page-header">
                    <div>
                        <h1 class="pm-page-title">
                            <i class="fa-solid fa-handshake" style="margin-right: 10px; font-size: 20px; color: var(--pm-slate-700);"></i>
                            Flyrobe Commission Records
                        </h1>
                        <p class="pm-page-subtitle">
                            Monitor partner commissions, GST breakdowns, beautician deductions, and net revenue settlements between Flyrobe and Sri Shringarr.
                        </p>
                    </div>
                    <div style="display: flex; gap: 8px; align-items: center;">
                        ...
                        <a href="flyrobe.php?export=1&<?= http_build_query($_GET) ?>" class="pm-btn pm-btn-primary" id="btnExportCSV">
                ...
                <div class="pm-kpi-grid">
                    <div class="pm-kpi-card">
                        ...
                        <div class="pm-kpi-value">₹ <?= number_format($summary['total_rent']) ?></div>
                        <div class="pm-kpi-sub">
                            Total product rental amount across <?= number_format($summary['total_rows']) ?> records
                        </div>
                    </div>
                    ...
                    <div class="pm-kpi-card">
                        ...
                        <div class="pm-kpi-sub">Tax component (3% / 12% / 18%)</div>
                    </div>
                    ...
                    <div class="pm-kpi-card">
                        ...
                        <div class="pm-kpi-sub">After GST & Beautician discount</div>
                    </div>
                </div>
                ...
                <div class="pm-card">
                    <div class="pm-card-body">
                        <form id="filterForm" method="GET" action="flyrobe.php">
                            <div class="pm-filter-grid">
                                ...
                                <div class="pm-form-group" style="min-width: 180px;">
                                    <label class="pm-label" for="filterQ">SKU / Invoice / Customer</label>
                                ...
                                <div style="display: flex; gap: 8px;">
                                    ...
                                    <a href="flyrobe.php" class="pm-btn pm-btn-secondary" title="Reset Filters">
                ...
                <div class="pm-card">
                        ...
                        <table class="pm-table" id="reportTable">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 95px;">Bill Date</th>
                                    <th style="width: 90px;">Type</th>
                                    <th style="width: 120px;">SKU</th>
                                    <th style="width: 130px;">Invoice No</th>
                                    <th>Customer Details</th>
                                    <th style="width: 140px;">Pick / Return Dates</th>
                                    <th class="text-right" style="width: 110px;">Rent Amount</th>
                                    <th class="text-center" style="width: 70px;">GST %</th>
                                    <th class="text-right" style="width: 95px;">GST (₹)</th>
                                    <th class="text-center" style="width: 95px;">Beautician</th>
                                    <th class="text-right" style="width: 110px;">Net Amount</th>
                            ...
                            <tbody>
                                <?php if (empty($report_rows)): ?>
                                    <tr>
                                        <td colspan="16" class="text-center" style="padding: 48px; color: var(--pm-slate-400);">
                                            <i class="fa fa-receipt" style="font-size: 32px; margin-bottom: 12px; display: block;"></i>
                                            No commission records found matching your filter criteria.
                                        </td>
                                    </tr>
                                <?php else: ?>
                                    <?php 
                                    ...
                                    foreach ($report_rows as $row): 
                                        $m = calculate_flyrobe_item_metrics($row);
                                        ...
                                        $b_disc_given = ($row['isBeauticianDiscountGiven'] == 1);
                                        $b_rate = $row['beautician_discountRate'] ? ($row['beautician_discountRate'] . $row['beautician_discountType']) : '';

                                        $search_text = strtolower($row['sku'] . ' ' . $bill_no . ' ' . $cust_name . ' ' . $cust_phone);
                                    ?>
                                        <tr data-search="<?= htmlspecialchars($search_text) ?>">
                                            ...
                                            <td>
                                                <a href="./rent_report_detail.php?id=<?= urlencode($row['purchase_id']) ?>" 
                                                   target="_blank" 
                                                   class="font-mono" 
                                            ...
                                            <td class="font-mono text-muted" style="font-size: 11px;">
                                                <div><i class="fa fa-arrow-up" style="font-size: 9px; color: var(--pm-slate-400);"></i> <?= ($row['pick_date'] && $row['pick_date'] !== '0000-00-00') ? date('d M Y', strtotime($row['pick_date'])) : '—' ?></div>
                                                <div style="margin-top: 2px;"><i class="fa fa-arrow-down" style="font-size: 9px; color: var(--pm-slate-400);"></i> <?= ($row['delivery_date'] && $row['delivery_date'] !== '0000-00-00') ? date('d M Y', strtotime($row['delivery_date'])) : '—' ?></div>
                                            </td>
                                            <td class="text-right font-mono" style="font-weight: 600; color: var(--pm-slate-900);">
                                                ₹ <?= number_format($m['rent_amt']) ?>
                                            </td>
                                            ...
                                            <td class="text-center font-mono">
                                                <?php if ($b_disc_given): ?>
                                                    <span class="pm-badge-neutral" title="<?= htmlspecialchars($b_rate) ?>">
                                                        ₹ <?= number_format(floatval($row['beautician_discountAmount']), 2) ?>
                                                    </span>
                                                <?php else: ?>
                            ...
                            <tfoot class="pm-table-footer">
                                <tr>
                                    ...
                                    <td class="text-right font-mono">₹ <?= number_format($summary['total_rent']) ?></td>
                                    <td></td>
                                    <td class="text-right font-mono">₹ <?= number_format($summary['total_gst'], 2) ?></td>
                                    <td class="text-center font-mono">₹ <?= number_format($summary['total_beautician'], 2) ?></td>
                    ...
                    <div class="pm-pagination-bar">
                        ...
                        <div class="pm-page-links">
                            <a href="<?= get_flyrobe_page_url(1) ?>" class="pm-page-link <?= ($current_page <= 1) ? 'disabled' : '' ?>" title="First Page">
                                <i class="fa fa-angle-double-left"></i>
                            </a>
                            <a href="<?= get_flyrobe_page_url($current_page - 1) ?>" class="pm-page-link <?= ($current_page <= 1) ? 'disabled' : '' ?>" title="Previous Page">
                            ...
                            ?>
                                <a href="<?= get_flyrobe_page_url($p) ?>" class="pm-page-link <?= ($p === $current_page) ? 'active' : '' ?>">
                                    <?= $p ?>
                                </a>
                            <?php endfor; ?>
                            ...
                            <a href="<?= get_flyrobe_page_url($current_page + 1) ?>" class="pm-page-link <?= ($current_page >= $total_pages) ? 'disabled' : '' ?>" title="Next Page">
                                <i class="fa fa-angle-right"></i>
                            </a>
                            <a href="<?= get_flyrobe_page_url($total_pages) ?>" class="pm-page-link <?= ($current_page >= $total_pages) ? 'disabled' : '' ?>" title="Last Page">
