...
if (!empty($sku)) {
    ...
    $where_conditions[] = "pr.bill_id IN (SELECT bill_id FROM order_detail WHERE item_id LIKE '%$safe_sku%')";
}
...
if ($con) {
    $q_kpi = mysqli_query($con, "
        SELECT 
            ...
            COALESCE(SUM(pr.rent_amount), 0) as total_sum
        FROM phppos_rent pr
        WHERE $where_sql
    ");
    if ($q_kpi && $rkpi = mysqli_fetch_assoc($q_kpi)) {
        $total_rows     = (int)$rkpi['total_count'];
        $total_rent_sum = (float)$rkpi['total_sum'];
    }
}
...
// -------------------------------------------------------------------------
// 2. Fetch Parent Bill Records for Current Page
// -------------------------------------------------------------------------
$page_bills = [];
$page_bill_ids = [];
...
if ($con && $total_rows > 0) {
    $query = "
        SELECT pr.bill_id, pr.bill_date, pr.rent_amount, pr.new_bill_number, pr.company_name,
    ...
    if ($rent_sql) {
        while ($row = mysqli_fetch_assoc($rent_sql)) {
            $page_bills[] = $row;
...
if ($con && !empty($page_bill_ids)) {
    ...
    // Items from order_detail
    $q_items = mysqli_query($con, "
        SELECT od.bill_id, od.item_id AS sku, pi.category_type, od.qty, od.rent AS single_rent
...
foreach ($page_bills as $bill) {
    ...
    $b_gross = (float)$bill['rent_amount'];
    $page_gross += $b_gross;
        ...
        <div class="content-wrapper" style="padding: 1.25rem 1.5rem !important; background: #f8fafc; min-height: calc(100vh - 58px);">
            ...
            <div class="pm-header-toolbar">
                <div>
                    <h1 class="pm-page-title">
                        <i class="fa-solid fa-file-invoice-dollar" style="color: var(--pm-slate-700);"></i>
                        Billing Rent Data (GST / Tally View)
                    </h1>
                    <p class="pm-page-subtitle">Itemized rental invoices with slab-wise GST breakdown, taxable values, and Tally export</p>
                </div>
                <div class="pm-toolbar-actions">
                    ...
                    <a href="tally_rentData_export.php?<?= $query_params ?>" class="pm-btn pm-btn-primary">
            ...
            <div class="pm-kpi-grid">
                ...
                <div class="pm-kpi-card">
                    ...
                    <div class="pm-kpi-sub">Filtered rent invoices</div>
                </div>

                <!-- Card 2: Total Gross Rent -->
                <div class="pm-kpi-card">
                    <div class="pm-kpi-top">
                        <span class="pm-kpi-label">Filtered Gross Rent</span>
                        <div class="pm-kpi-icon"><i class="fa-solid fa-indian-rupee-sign"></i></div>
                    </div>
                    <div class="pm-kpi-value">₹ <?= number_format($total_rent_sum, 2) ?></div>
                    <div class="pm-kpi-sub">Cumulative rental turnover</div>
                </div>
                    ...
                    <form method="GET">
                        <div class="pm-filter-row">
                            <div class="pm-form-group">
                                ...
                                <input type="text" name="bill_id" class="pm-input" placeholder="e.g. 7194 or S-123" value="<?= htmlspecialchars($bill_id) ?>">
                            </div>
                            <div class="pm-form-group">
                            ...
                            <div style="display: flex; gap: 6px;">
                                ...
                                <?php if (!empty($bill_id) || !empty($from_date) || !empty($to_date) || !empty($sku) || !empty($company_name) || $limit != 20): ?>
                                    <a href="ssfs_format_rent_view.php" class="pm-btn pm-btn-outline" title="Reset Filters">
                                ...
                                <tr>
                                    <td colspan="12">
                                        <div class="pm-empty-state">
                                            ...
                                            <div class="pm-empty-title">No Rent Bills Found</div>
                                            <p class="pm-empty-desc">No rental transaction matches the selected filter criteria.</p>
                                        </div>
                                    </td>
                                </tr>
                            <?php else: ?>
                                ...
                                ?>
                                    ...
                                    <tr class="pm-bill-header-row" id="bill-row-<?= $b_id ?>">
                                        ...
                                        <td class="pm-bill-nowrap">
                                            <a href="./reports/rent_report_detail.php?id=<?= $b_id ?>" target="_blank" class="pm-bill-link" title="Open Rent Bill Details">
